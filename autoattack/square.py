    ...
    def attack_single_run(self, x, y):
        ...
        with torch.no_grad():
            ...
            if self.norm == 'Linf':
                ...
                real_accuracy = nn.Softmax(dim=1)(self.predict(x_best))[:, 0]
                if (margin_min < 0.0).all() and (real_accuracy > 0.9).all():
                    return n_queries, x_best

                for i_iter in range(self.n_queries):
                    # idx_to_fool = (margin_min > 0.0).nonzero().squeeze()
                    real_accuracy = nn.Softmax(dim=1)(self.predict(x_best))[:, 0]
                    idx_to_fool = torch.logical_or((margin_min > 0.0), (real_accuracy < 0.9)).nonzero().squeeze()
                    ...
                    x_new = torch.clamp(x_new, 0, 1)  # TODO: Change to range of the image
                    x_new = self.check_shape(x_new)

                    margin, loss = self.margin_and_loss(x_new, y_curr)
                    new_real_accuracy = nn.Softmax(dim=1)(self.predict(x_new))[:, 0]
                    # update loss if new loss is better
                    idx_improved = (loss < loss_min_curr).float()
                    # idx_improved = torch.logical_and((loss < loss_min_curr),
                    #                                  (new_real_accuracy > real_accuracy)).float()
                    ...
                    # idx_miscl = torch.logical_and((margin <= 0.), (new_real_accuracy > real_accuracy)).float()
                    ...
                    ind_succ = torch.logical_and((margin_min <= 0.0), (real_accuracy >= 0.9)).nonzero().squeeze()
                    ...
                    if self.verbose and ind_succ.numel() != 0:
                        print('{}'.format(i_iter + 1),
                              ...
                              '- real_accuracy=', real_accuracy.detach().cpu().numpy()
                              )

                    if ind_succ.numel() == n_ex_total and (real_accuracy > 0.9).all():
                        break
